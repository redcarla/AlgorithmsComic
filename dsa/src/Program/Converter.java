...
public class Converter {
    ...
    public static void Converter1(String newCSV) throws Exception {                   //convert csv using split
        ...
        if (newCSV != null) {
            ...
            if (max >= 1) {character.setName(splitData[1].trim());} //filling objects with data from csv
            if (max < 3) {character.setId(splitData[3].trim());}
            if (max >= 4) {character.setAlign(splitData[4].trim());}
            if (max >= 5) {character.setEye(splitData[5].trim());}
            if (max >= 6) {character.setHair(splitData[6].trim());}
            if (max >= 7) {character.setSex(splitData[7].trim());}
            if (max > 8) {character.setGSM(splitData[8].trim());}
            if (max > 9) {character.setAlive(splitData[9].trim());}
